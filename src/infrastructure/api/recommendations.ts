    ...
    export const getMovieRecommendations = async (movieId: number): Promise<RecommendationsResponse> => {
    const { data } = await tmdbClient.get(`/movie/${movieId}/recommendations`);
    const parsed = RecommendationsResponseSchema.safeParse(data);

    if (!parsed.success) {
        throw new DomainValidationError(`Error de validación en recommendations: ${parsed.error.message}`);
    }
